...
class LoginPage(BasePage):
    ...
    SELECTORS_HISTORY = {
        "USERNAME": SelectorRecord(
            selector="input[placeholder*='用户名'], input[placeholder*='户名']",
            candidates=["input[placeholder*='用户名']", "input[placeholder*='户名']"],
            status="verified",
            stability="high",
        ),
        "PASSWORD": SelectorRecord(
            selector="input[placeholder*='密码']",
            candidates=["input[placeholder*='密码']"],
            status="verified",
            stability="high",
        ),
        "VERIFY_CODE": SelectorRecord(
            selector="input[placeholder*='验证码']",
            candidates=["input[placeholder*='验证码']"],
            status="verified",
            stability="high",
        ),
        "VERIFY_CODE_IMAGE": SelectorRecord(
            selector="img[src*='code'], img:nth-of-type(2)",
            candidates=["img[src*='code']", "img:nth-of-type(2)"],
            status="verified",
            stability="low",
        ),
        "LOGIN_BUTTON": SelectorRecord(
            selector="button:has-text('登录'), input[type='submit']",
            candidates=["button:has-text('登录')", "input[type='submit']"],
            status="verified",
            stability="high",
        ),
        "FORGOT_PASSWORD": SelectorRecord(
            selector="button:has-text('忘记密码')",
            candidates=["button:has-text('忘记密码')"],
            status="verified",
            stability="high",
        ),
        "ERROR_MESSAGE": SelectorRecord(
            selector=".error-message, .msg-error, .tips-error",
            candidates=[".error-message", ".msg-error", ".tips-error"],
            status="verified",
            stability="medium",
        ),
        "IDENTITY_DIALOG": SelectorRecord(
            selector="text=您有多身份，请选择",
            candidates=["text=您有多身份，请选择"],
            status="verified",
            stability="high",
        ),
        "IDENTITY_BUTTON": SelectorRecord(
            selector="button:has-text('{identity}'), div:has-text('{identity}')",
            candidates=["button:has-text('{identity}')", "div:has-text('{identity}')"],
            status="verified",
            stability="high",
        ),
    }
    ...
    def login(
        ...
        self.fill(self.USERNAME_INPUT, username, "用户名")
        self.fill(self.PASSWORD_INPUT, password, "密码")
        self.fill(self.VERIFY_CODE_INPUT, verify_code, "验证码")
        ...
        try:
            ...
            for btn in buttons:
                ...
                if (
                    text
                    and len(text) > 2
                    and ("管理员" in text or "教师" in text or "学生" in text)
                ):
